...
Properties {
	...
	SubShader {
		...
		v2f vert (appdata_full v) {
			...
			o.viewDir=ObjSpaceViewDir(v.vertex);
			o.uv=TRANSFORM_TEX(v.texcoord, _MainTex);
			o.atten=LIGHT_ATTENUATION(v);//获取光强
		...
		float4 frag(v2f i):COLOR
		{
			...
			float toon=floor(diff*i.atten*_Steps)/_Steps/dist/dist;
			diff=lerp(diff,toon,_ToonEffect);

			half3 h = normalize (lightDir + viewDir);//求出半角向量
			float nh = max (0, dot (N, h));
			float spec = pow (nh, 32.0);//求出高光强度
			float toonSpec=floor(spec*i.atten*2)/ 2;//把高光也离散化
			spec=lerp(spec,toonSpec,_ToonEffect);//调节卡通与现实高光的比重

			
			c=c*_LightColor0*(diff+spec);//求出最终颜色
			return c;
		}
		ENDCG
		}
