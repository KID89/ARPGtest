...
Properties {
		...
		_RimPower("RimPower",range(0,2))=0.5//边缘光强度
		_ToonRimStep("Steps of ToonRim",range(0,9))=3//边缘光色阶层数
	}
	SubShader {
		pass{//处理光照前的pass渲染
		...
		float4 frag(v2f i):COLOR
		{
			...
			c=c*_LightColor0*(diff)*rim;//把最终颜色混合
			return c;
		}
		ENDCG
		}//
		...
		float4 frag(v2f i):COLOR
		{
			float4 c=c=tex2D(_MainTex, i.uv);
			float3 N=normalize(i.normal);
			float3 viewDir=normalize(i.viewDir);
